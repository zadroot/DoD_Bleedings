...
#define WEAPON_CRIT    -1
#define DOD_MAXPLAYERS 33 // Maximum players that DoD:S support
...
{
	// FCVAR_DONTRECORD don't saves version convar in plugin's config
	...
	// Hook spawn event and damage event to properly set bleedings (disable/enable respectively)
...
{
	// Check whether or not timer is running
			...
			if (bool:BleedInfo[i][enabled] == true)
			{
				...
				switch (GetConVarBool(Bleed_Mode))
				{
					case false:
					{
						...
						{
							// ...take X damage (depends on unique value) every Y seconds by unspecified weapon
						...
						{
							// But if player is having more health, stop bleeding
							StopBleed(i);
						}
					}
